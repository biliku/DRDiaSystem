...
MODEL_SAVE_PATH = "django\DRDiaSys\diagnosis\\best_lesion_segmentation_model.pth"
...
def main():
    ...
    # 修复：正确使用类别权重
    class_weights = torch.tensor([0.1, 0.9, 0.9, 0.8, 1.0], device=DEVICE)
    print(f"使用的类别权重: {class_weights}")
    
    loss_fn = CombinedLoss(class_weights, dice_weight=0.5, ce_weight=0.5)  # 修复：传入权重
